...
class tableDataObject{
    ...
    public function __construct($objectid = null){
        ...
        if(isset($objectid)){
            ...
            if((!$getarecord) || count($getarecord) !== 1){
                die(_("Error: Zero or multiple records returned when trying to create a tableDataObject!"));
            } else {
    ...
    public static function getRecordByParams($table, $conditions,$returnarray = false){
        ...
        if(!is_array($conditions)){
            die("Error: conditions for finding an object must be a key => value pair array");
        }
        ...
        $rowbyparam = $payrolldb->resultSet();
    ...
    public static function listAll($createObjects = false){
	    $objClass = get_called_class();
        ...
        global $payrolldb;
        $payrolldb->prepare("select * from $tablename order by $tableinfo->primaryKey");
        if (!$createObjects){
        	$results = $payrolldb->resultSet();
        } else {
        	foreach ($payrolldb->resultSet() as $resultrecord){
    ...
    public function store($forcenulls = false){
        ...
        if(!isset($this->tableName) || !isset($this->recordObject)){
            die (_("Error storing tableDataObject - missing tableName or recordObject"));
        }
        ...
        foreach ($this->recordObject as $field => $value){
             ...
             * like with the MarketplaceJob. It breaks other places where we need implicit nulls
        ...
        for ($fcount = 0; $fcount < count($fields); $fcount++) {
            if (isset($values[$fcount])) {
                $payrolldb->bind($binders[$fcount], $values[$fcount]);
            }
        }
        if ($payrolldb->execute()){
            if($mode == 'insert'){
                $this->recordObject->$pk = $payrolldb->lastInsertId();
            }
            return $this;
        } else {
            /*
             ...
             * or update.
             */
            die (_("error in inserting or updating tableDataObject"));
        }
